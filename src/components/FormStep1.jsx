...
export default function FormStep1({ userFormValues, userFormErrors: errors, handleUserFormChange: handleChange }) {
  ...
  return (
    <form
      ...
      <h1 className="text-3xl font-bold text-marineBlue mb-2">Personal info</h1>
      ...
      <div className="my-5 flex flex-col relative">
        ...
        <span className={`${errors.name ? '' : 'hidden'} transition-all text-strawberryRed font-medium md:absolute right-0`}>This field is required</span>
        <input
          className={`${errors.name ? 'border-strawberryRed' : ''} transition-all transition-all outline-none font-medium text-marineBlue mt-1 px-3 py-3 rounded-lg border-solid border-1 focus:border-purplishBlue cursor-pointer`}
      ...
      <div className="my-5 flex flex-col relative">
        ...
        <span className={`${errors.email ? '' : 'hidden'} transition-all text-strawberryRed font-medium md:absolute right-0`}>This field is required</span>
        <input
          className={`${errors.email ? 'border-strawberryRed' : ''} transition-all outline-none font-medium text-marineBlue mt-1 px-3 py-3 rounded-lg border-solid border-1 focus:border-purplishBlue cursor-pointer`}
          type="email" name="email" id="email" placeholder="e.g. stephenking@lorem.com" defaultValue={email} onChange={handleChange} />
      </div>

      <div className="my-5 flex flex-col relative">
        <label className="text-marineBlue" htmlFor="number">Phone Number</label>
